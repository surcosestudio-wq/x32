...
void X32Config::DefineMixerparameters() {
   ...
   DefParameter(CHANNEL_LCD_MODE, cat, "LCD Mode")
    ...
    DefParameter(CHANNEL_LOWCUT_FREQ, cat, "Lowcut 2404", MAX_VCHANNELS)
    ->DefNameShort("LC Fr")
    ->DefUOM(MP_UOM::HZ)
    ...
    // Reuse the four existing PEQ sections: LOW, MID, HIGH and AIR.
    // Filter type and Q are fixed internally to the original plugin values.
